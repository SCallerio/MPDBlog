...
Gulf of Mexico (GoM) using publicly available BSEE Application for Permit to Drill (APD) data. It demonstrates how to 
download the GoM wells permit data, visualize recent approvals on an interactive rig-tracking map with Plotly, and 
generate operator- and rig-level summary tables relevant to offshore drilling and MPD planning. Moreover, it details an extensive exploratory data analysis of the BSEE APD database, with details related to drilling activity (inferred from the permit entries) for the GoM operators and rigs.*
...
<p>Unique/distinct well count by operator, including every permit type present on the dataset: 'Revised Sidetrack', 'Revised New Well', 'Revised Bypass', 'Bypass', 'Sidetrack', 'New Well'.</p>
...
<p>Unique/distinct rig count by rig category (`RIG_TYPE_CODE` field), with the mean declared water depth for each rig category.</p>
